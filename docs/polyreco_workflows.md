# Справочник PolyReco: рецепты и история

Этот файл сохраняет материалы прежнего AGENTS.md после сокращения постоянных инструкций.
Читать только раздел, относящийся к текущей задаче; команды ниже не являются обязательным checklist.
Все команды выполняются из корня проекта. Перед запуском проверить актуальный CLI и целевые пути.
Описания реализации, слова «текущий/сейчас», результаты и планы ниже относятся к историческому состоянию,
если явно не сказано иное. При расхождении действуют корневой [AGENTS.md](../AGENTS.md), текущий код
и полные параметры выбранного артефакта. Не запускать расчёты для «подтверждения» архивных чисел.

Сохранены исходные рецепты, результаты и гипотезы; исправлена изоляция smoke-кэша,
а старые freeze-входы и RMS-roadmap явно обозначены как исторические.
...
```bash
uv run python src/generate_full_circle_split_cached_viewer.py --models round --max-contours 50 --no-progress --cache-json output/smoke_full_circle_split_cache.json --output-html output/smoke_full_circle_split_viewer.html
```
...
## Архив RMS-эксперимента

Исходный файл эксперимента: `src/reconstruct_faces_from_rms_regions.py`.
Сейчас часть реализации вынесена в `src/polyreco/rms_*.py`; актуальная карта находится в корневом AGENTS.md.
Ниже — раннее состояние эксперимента, не описание нынешнего golden.
...
Что было реализовано в описываемой версии `src/reconstruct_faces_from_rms_regions.py`:
...
Историческая команда базового эксперимента (не воспроизводит поздний golden):
...
Параметры описываемого исторического этапа (актуальные defaults сверять с parser):
...
### Историческая команда packaging/freeze

Это запись прежнего запуска, **не готовая текущая команда**: указанные `/private/tmp` входы могут отсутствовать.
Перед freeze нужны реально существующие проверенные reconstruction JSON; не подставлять другие результаты
и не запускать reconstruction только ради пропавших temp-файлов. Для вывода общего viewer сначала проверить
embedded payload существующего benchmark JSON. Для нового freeze проверить input paths, source hashes и параметры.
Сохранённый пример:
...
### Исторический результат раннего `round`

Числа `442 vertices / 100 faces` ниже не являются нынешним baseline или критерием regression:
...
Архивные выходные файлы этого этапа:
...
Исторический результат fixed `window=10`:
...
Что проверялось на архивном этапе (не инструкция повторить проверки):
...
Выводы и проблемы на архивном этапе:
...
### Исторические гипотезы дальнейших улучшений

Это архивный план, **не текущее поручение**. Часть пунктов позднее реализована или опровергнута.
Не начинать эти работы без отдельной задачи и проверки актуального кода/результатов.
...
Архивные команды (запускать только по текущей задаче, не автоматически):
